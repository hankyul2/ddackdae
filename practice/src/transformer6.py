...
class Transformer(nn.Module):
    ...
    @torch.no_grad()
    def greedy_decode(self, src, src_mask, max_len=20, BOS=1, EOS=-1):
        src_rep = self.encode(src, src_mask)
        ys = torch.full((1, 1), BOS, dtype=torch.long, device=src.device)
        for iter in range(max_len-1):
            prop = self.generator(self.decode(src_rep, ys, *get_mask(src, ys, 0)))[:, -1]
            _, next_word = prop.max(dim=-1, keepdim=True)
...
def get_model(src_vocab_size=10, tgt_vocab_size=10, d_model=512, dropout=0.1, h=8, d_ff=2048, N=6):
    c = copy.deepcopy
...
def get_sample_data(high=10, batch_size=30, seq_len=10, pad_idx=0):
    src = torch.randint(1, high, size=(batch_size, seq_len)).long()
    src[:, 0] = 1
...
def get_mask(src, tgt_input, pad_idx):
    src_mask = get_pad_mask(src, pad_idx)
    tgt_mask = get_pad_mask(tgt_input, pad_idx) | get_seq_mask(tgt_input)
    return src_mask, tgt_mask


def get_pad_mask(src, pad_idx=0):
    return (src == pad_idx).unsqueeze(1).unsqueeze(1)


def get_seq_mask(tgt_input):
    return torch.triu(torch.ones((tgt_input.size(1), tgt_input.size(1)), device=tgt_input.device), diagonal=1) == 1